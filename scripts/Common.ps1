...
function New-AzureStorageContainers
{
    ...
    foreach($AzureStorageContainer in $storageContainerList)
    {
        ...
        $status = Get-StorageContainerStatus -ContainerName $AzureStorageContainer
        ...
        Log-Message "Updating Storage Container Permissions (Cli: $AzureCliVersion): $($AzureStorageContainer)" -Context "Update Storage Containers Permissions"
        ...
        if ($AzureCliVersion -eq "1" )
        {
            ...
            azure storage container set --account-name $StorageAccountName --account-key $StorageAccountKey --container $AzureStorageContainer --permission $AccessPolicy --json
        }
        else 
        {
            # Azure Cli 2.0
            ...
            if ($AzureStorageConnectionString)
            {
                ...
                az storage container set-permission --account-name $StorageAccountName --account-key $StorageAccountKey --name $AzureStorageContainer --public-access $AccessPolicy --connection-string $AzureStorageConnectionString -o json
            }
            else
            {
                az storage container set-permission --account-name $StorageAccountName --account-key $StorageAccountKey --name $AzureStorageContainer --public-access $AccessPolicy -o json
            }
        }
    }
}
