...
# Setup mysql backup
if [[ $mysql_admin_username != unused ]] ; then
...
# Setup mongo backup
if [[ $mongo_admin_username != unused ]] ; then
...
setup_backup \
    "${installer_basepath}/configuration_${database_type}.sh" \
    ...
    "${cluster_admin_email}" \
    "${azure_cli_version}" \
    "${encoded_backup_storageaccount_endpoint_suffix}" \
    "${mongo_backup_username}" \
    "${mongo_backup_password}"
