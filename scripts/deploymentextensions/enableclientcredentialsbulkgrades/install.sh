...
#
# This script enables Client Credentials & Bulk Grades on Ficus for existing clusters. 
# These changes enable deep integration principally for Microsoft Professional Program (MPP).
# Before running this deployment extension, the user is expected to confirm that:
# 1. it is being run against Ficus x.x version of openedx as available from http://github.com/microsoft/edx-platform oxa/master.fic branch
# 2. this script has not been run before against the target azure cluster
# 3. [comments about azure]
# [ Text block from Team ]
